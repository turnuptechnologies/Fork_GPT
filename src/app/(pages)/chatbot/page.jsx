...
export default function Chat() {
  const { messages, input, handleInputChange, handleSubmit, isFetching } =
    useChat(); // Added isFetching
  ...
  const renderResponse = () => {
    ...
    useEffect(() => {
      ...
      if (lastMessage?.role === "bot" && !isFetching) {
        setIsTyping(true);
        setAnimatedMessage(""); // Reset the animated message
      ...
      <div className="flex-1 overflow-y-auto space-y-4 p-4 rounded-lg shadow-inner bg-white">
        {messages.map((m, index) => {
          const isLastBotMessage =
            m.role === "bot" && index === messages.length - 1;
          ...
          return (
            ...
            >
              ...
              >
                {/* {isFetching && m.role === "bot" && index === messages.length && <p>Loading...</p>} */}
                <>
                  {isLastBotMessage ? (
                    // Show animated response after fetching
                    <>
                      <p>{animatedMessage}</p>
                    </>
                  ) : (
                    // Static content for older messages
                    <p>{m.content}</p>
                  )}
                </>
              </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="submit"
          className={`bg-blue-500 text-white px-6 py-3 rounded-full shadow-md transition-transform transform hover:scale-105 ${
            isFetching ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-600"
          }`}
          disabled={isFetching} // Disable button when fetching
        >
          {isFetching ? "Loading..." : "Send"} {/* Optional: Change label */}
        </button>
      </form>
    </div>
  );
